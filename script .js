...
const PHASES = {
  1: {
    ...
    goal: 120,
    items: [
      { emoji: "❤️", pts: 10, size: 48, speed: () => rand(260, 400) },
      { emoji: "💖", pts: 15, size: 52, speed: () => rand(300, 450) },
      { emoji: "💘", pts: 20, size: 44, speed: () => rand(320, 480) },
      { emoji: "💣", pts: -30, size: 44, speed: () => rand(350, 520) }
    ],
    weights: [35, 25, 18, 22],
    spawnMs: 420
  },
  ...
  2: {
    ...
    goal: 150,
    items: [
      { emoji: "⭐", pts: 15, size: 48, speed: () => rand(330, 500) },
      { emoji: "✨", pts: 20, size: 44, speed: () => rand(360, 540) },
      { emoji: "🌟", pts: 25, size: 52, speed: () => rand(300, 470) },
      { emoji: "💣", pts: -35, size: 44, speed: () => rand(420, 600) }
    ],
    weights: [30, 25, 18, 27],
    spawnMs: 320
  }
...
function answerQuiz(choice) {
  ...
  if (acertou) {
    ...
    if (quizIndex === 0) {
      ...
      document.getElementById("messageText").textContent = "muita saudade dela amor";
    } else {
    ...
    if (quizIndex === 0) {
      ...
      document.getElementById("messageText").textContent = "poxa amor :(";
    } else {
...
function startPatienceTest() {
  ...
  // Checkpoints onde o troll acontece (em %)
  const trollPoints = [40, 65, 85];
  const trollMessages = [
    "Ih, escorregou kkkk. Começa de novo não, continua daí 😂",
    "KKKKKK caiu de novo?? tô rindo muito de você 😭",
    "NÃO ACREDITO. Tava tão perto… segura direito amor 💀"
  ];
  const trollResets = [12, 8, 5]; // volta pra esse % em cada troll

  ring.style.strokeDashoffset = CIRCUM;
  emoji.textContent = "";
  pText.textContent = "Segure o botão até completar. Se soltar, regride bastante. 😈";
  ...
  function updateRing() {
    ...
    if (pct > 0.75) emoji.textContent = "❤️";
    else if (pct > 0.45) emoji.textContent = "💗";
    else if (pct > 0.2) emoji.textContent = "🤍";
  ...
  function onDown(e) {
    ...
    holdInterval = setInterval(() => {
      holdProgress += 1.4; // sobe mais devagar

      // Verifica trolls nos checkpoints
      if (trollCount < trollPoints.length && holdProgress >= trollPoints[trollCount]) {
        const idx = trollCount;
        trollCount++;
        holdProgress = trollResets[idx];
        emoji.textContent = "";
        pText.textContent = trollMessages[idx];
        updateRing();
        return;
      }
      ...
      if (holdProgress >= 100) {
        clearInterval(holdInterval);
        emoji.textContent = "❤️";
        pText.textContent = "Ufa!! Finalmente. Você tem uma paciência enorme amor 😂❤️";
  ...
  function onUp() {
    clearInterval(holdInterval);
    // Regressão muito maior ao soltar
    holdProgress = Math.max(0, holdProgress - 22);
    updateRing();
    if (holdProgress < trollPoints[Math.max(0, trollCount - 1)] || trollCount === 0) {
      pText.textContent = "Soltou?? Que isso amor, segura! 😤";
    }
  }
...
function showQuestionScreen() {
  ...
  btn.style.left = (window.innerWidth  * 0.55) + "px";
  btn.style.top  = (window.innerHeight * 0.72) + "px";

  // Clonar para remover listeners anteriores caso jogue de novo
  const fresh = btn.cloneNode(true);
  btn.parentNode.replaceChild(fresh, btn);
  fresh.style.left = btn.style.left;
  fresh.style.top  = btn.style.top;
  ...
  fresh.addEventListener("touchstart", (e) => {
    ...
    e.stopPropagation();
    const p = randomPos();
    fresh.style.left = p.x + "px";
    fresh.style.top  = p.y + "px";
  }, { passive: false });
